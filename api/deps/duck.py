...
def con() -> duckdb.DuckDBPyConnection:
    ...
    """)
    c.execute(f"CREATE VIEW dim_product AS SELECT ProductId AS product_id, ProductName AS product_name, ProductCode AS product_code, CatLvl2_Name AS category, ListPrice AS list_price FROM read_parquet('{dp}')")
    c.execute(f"CREATE VIEW dim_branch  AS SELECT BranchId AS outlet_id, BranchName AS outlet_name, BranchCode AS outlet_code, Segment AS brand, ChannelId AS channel_id, Address AS address FROM read_parquet('{db}')")
    c.execute(f"CREATE VIEW backtest AS SELECT CAST(date AS DATE) AS date, BranchId AS outlet_id, ProductId AS product_id, CAST(y AS DOUBLE) AS actual, CAST(p50 AS DOUBLE) AS expected, CAST(p85 AS DOUBLE) AS safe FROM read_parquet('{bt}')")
    _con = c
    return _con
